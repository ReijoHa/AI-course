...
def shortest_path(source, target):
    ...
    while True:
        ...
        nodes = neighbors_for_person(current.state)

        for person in nodes:

            if person[1] == target:
                node = Node(state=person[1], parent=current, action=person[0])
                ...
                if not (person[1] in seen):
                    add = Node(state=person[1], parent=current, action=person[0])
                    que.add(add)
